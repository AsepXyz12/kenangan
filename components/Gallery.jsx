...
export default function Gallery({ photos, years }) {
  ...
  // Foto-foto dalam grup dengan tanggal yang sama cukup menampilkan jam
  // unggahnya (real time), bukan tanggal berulang.
    ...
    <section className="max-w-5xl mx-auto px-6 pb-20">
      <div className="flex flex-wrap items-center gap-3 mb-10">
        <input
          ...
          className="font-body text-sm bg-white/70 border border-emerald/20 px-3 py-2 flex-1 min-w-[200px] focus:bg-white"
        />
        <select
          value={year}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-24 border border-dashed border-emerald/20">
          ...
          <p className="mt-2 text-sm text-ink/50">
            Belum ada foto yang cocok. Admin bisa mulai mengunggah kenangan pertama.
          </p>
        </div>
      ) : (
